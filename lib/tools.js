...
module.exports = {
  ...
  checkBashrc: function(game) {
    let grep;
    try {
      const command = `grep '${game.bashrc(game.path)}' ~/.bashrc`;
      grep = execSync(command);
    } catch (error) {}

    if (game.bashrc && !grep) {
      console.log('You need run the following commands:');
      console.log('');
      console.log(`echo "${game.bashrc(game.path).replace(/"/g, '\\"').replace(/\$/g, '\\$')}" >> ~/.bashrc`);
      console.log('. ~/.bashrc');
      console.log('');
      process.exit(1);
    }
