    ...
    rcon.onmessage = function(msg) {
      ...
      if (data.Message && data.Identifier >= 0 && !ignoreRow(data.Message)) {
        ...
        if (data.Message === 'Server startup complete') {
          ...
          tools.text(
            'Server startup took: ' + tools.secondsToString(startupTime)
          );
          checkUpdatesTimeout();
        }
      }
...
function ignoreRow(message) {
  return !!(
    ...
    message.match(
      /^(Saved .* ents, serialization.*, write.*, disk.* totalstall)/i
    )
  );
}
...
function rconSay(message, time) {
  return new Promise(function(resolve, reject) {
    rconCmd('say ' + message, time).then(function() {
      resolve(true);
    });
  });
}
...
function checkUpdates() {
  ...
  cd(thisGame.path);
  var latest_file = execSync(
    'ls -tr ' + thisGame.path + '/app_info_*.txt | tail -n 1'
  )
    .toString()
    .replace(/\n/, '');
  var new_file =
  ...
  execSync(
    './steamcmd.sh +login anonymous +app_info_update 1 +app_info_print ' +
      thisGame.appId +
      ' +quit > ' +
      new_file
  );
...
function parseFile(file) {
  var parsed = execSync(
    'grep -i -A 1000 "branches" ' + file + ' | grep "public" -A 4'
  );
...
function previousUpdateMessage() {
  ...
  if (previousUpdateTime) {
    return (
      ', previous update took ' + tools.secondsToString(previousUpdateTime)
    );
  }
